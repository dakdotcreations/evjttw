...
<a
		...
		{#if tags.length > 0}
			<div class="absolute left-3 top-3 flex flex-wrap gap-1.5">
				{#each tags.slice(0, 2) as tag (tag.slug)}
					<span
						class="bg-black/55 px-2.5 py-1 text-xs font-semibold text-white backdrop-blur-sm">
						{tag.name}
					</span>
				{/each}
			</div>
	...
	<div class="py-4">
		...
		<h3
			class="font-display text-2xl leading-tight tracking-wide text-black transition-colors group-hover:text-primary">
			{title}
		</h3>
